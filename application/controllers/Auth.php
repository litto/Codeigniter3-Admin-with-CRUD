...
// Passing to Theme
		...
		$notificationbar = $this->load->view("admin_template/admin_notification" , $data , true);
	...
	public function user_list()
       ...
       for($i=0;$i<$cnt;$i++){
        ...
        if(count($list)>0){
       $this->adminuser_model->unpublish_records($list);
       ...
       $this->adminuser_model->publish_records($list);
       ...
       for($i=0;$i<$cnt;$i++){
        ...
        if(count($list)>0){
       $this->adminuser_model->delete_records($list);
       ...
       redirect('auth/user_list', 'refresh');
            ...
            $data['model_admingroup'] = $this->admingroup_model; 
            $data['currentpage']="home";
			$data['title'] = $this->lang->line('index_heading');
			$data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
            $loggeduser = $this->ion_auth->user()->row();
            ...
            //list the users
			//$data['users'] = $this->ion_auth->users()->result();
			$data['usergroups']=$this->admingroup_model->get_all();
            $searchstring='';
    ...
    // Fetch Records 
	     ...
	     $totalrecords= $this->adminuser_model->get_allusers($searchparameters);
	     $data['users']=$this->adminuser_model->get_users($per_page_records,$page,$searchparameters);
	     $data['count']=count($totalrecords);
	//Pagination
	$config['base_url'] = $data['urls'].'/auth/user_list/';
...
public function unpublish_user(){
        ...
        $data = array('active' =>'0');
   $this->session->set_flashdata('warning', 'Selected Items Unpublished');
        $this->adminuser_model->update_record($id, $data);
...
public function del_user(){
        ...
        $data = array();
        $id = $this->input->get('id');
        $this->adminuser_model->delete_record($id);
         $this->session->set_flashdata('error', 'Selected Items Deleted');
        echo '<script type="text/javascript">location.reload();</script>';
	...
	public function change_password()
	{
     ...
     $this->data['currentpage']='changepassword';
     $header=$this->load->view("admin_template/admin_mainheader",$this->data, true);
		$sidemenu = $this->load->view("admin_template/admin_sidebar" ,$this->data , true);
		$topheader = $this->load->view("admin_template/admin_topbar" ,$this->data , true);
		$footer = $this->load->view("admin_template/admin_footer" ,$this->data , true);
		 $this->data['header']=$header;
	    $this->data['sidemenu']=$sidemenu;
	    $this->data['topheader']=$topheader;
	...
	public function create_user()
	{
		...
		else
		{
            ...
            $this->data['currentpage']="create_user";
            $loggeduser = $this->ion_auth->user()->row();
            $this->data['user']=$loggeduser;
         $header=$this->load->view("admin_template/admin_mainheader",$this->data, true);
		$sidemenu = $this->load->view("admin_template/admin_sidebar" ,$this->data , true);
	...
	*/
	...
	public function edit_user($id)
	{
        ...
        $this->data['id']=$id;
		$this->data['currentpage']="edit_user";
